...
.lumify-widget-root {
    --lw-accent: ${t};
    --lw-accent-hover: ${((e,t)=>{let i=e.replace("#",""),o=(3===i.length&&(i=i.split("").map(e=>e+e).join("")),parseInt(i,16)),r=(o>>16)+t,n=(o>>8&255)+t,a=(255&o)+t;return r=Math.max(0,Math.min(255,r)),n=Math.max(0,Math.min(255,n)),a=Math.max(0,Math.min(255,a)),"#"+((1<<24)+(r<<16)+(n<<8)+a).toString(16).slice(1)})(t,-15)};
    --lw-z-index: ${e.zIndex};
...
`}function a(e){var t=document.createElement("div");return t.textContent=e,t.innerHTML}function l(){return 0<=navigator.platform.toUpperCase().indexOf("MAC")?"⌘K":"Ctrl+K"}function r(e,t){var i=a(t.url||""),o=a(t.title||t.page_title||(e=>{if(!e)return"Source";try{return new URL(e).hostname.replace(/^www\./,"")}catch(e){return"Source"}})(t.url)),r=(t=>{if(!t)return"";try{var i=new URL(t);let e=i.pathname;return 30<e.length&&(e=e.substring(0,27)+"..."),i.hostname+e}catch(e){return 40<t.length?t.substring(0,37)+"...":t}})(t.url);return`<a href="${i}" target="_blank" rel="noopener" class="lumify-widget-citation" data-citation="${e}">
            ${e}
            <span class="lumify-widget-citation-tooltip">
                <span class="lumify-widget-citation-tooltip-content">
                    ...
                    <span class="lumify-widget-citation-info">
                        <span class="lumify-widget-citation-title">${o}</span>
                        <span class="lumify-widget-citation-url">${r}</span>
                    </span>
                </span>
            </span>
        </a>`}function e(){var t=document.querySelectorAll('script[src*="lumify-widget"]'),t=t[t.length-1];if(t){var i=t.src||"";let e="";i&&(i=new URL(i),e=i.origin);i={apiKey:t.dataset.apiKey||t.getAttribute("data-api-key"),appId:t.dataset.appId||t.getAttribute("data-app-id"),mode:t.dataset.mode||"floating",position:t.dataset.position||"bottom-right",theme:t.dataset.theme||"auto",accentColor:t.dataset.accentColor||"#6366f1",buttonText:t.dataset.buttonText||"",buttonIcon:t.dataset.buttonIcon||"search",placeholder:t.dataset.placeholder||"Ask anything...",keyboardShortcut:"false"!==t.dataset.keyboardShortcut,showBranding:"false"!==t.dataset.showBranding,zIndex:parseInt(t.dataset.zIndex)||9999,emptyText:t.dataset.emptyText||"Ask a question to get started",modalTitle:t.dataset.modalTitle||"Search",triggerSelector:t.dataset.triggerSelector,containerSelector:t.dataset.containerSelector,baseUrl:e};i.apiKey?i.appId?(window.LumifyWidget=new o(i),window.LumifyWidget._initialized=!0):console.error("LumifyWidget: data-app-id is required"):console.error("LumifyWidget: data-api-key is required")}else console.error("LumifyWidget: Script tag not found")}"loading"===document.readyState?document.addEventListener("DOMContentLoaded",e):e()}})();